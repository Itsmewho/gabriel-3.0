...
class TickProcessor:
    ...
    def __init__(
        ...
        for i in range(self.n):
            self.dwx.open_order(symbol=symbol, order_type="buylimit", lots=0.01, price=entry_price)  # type: ignore
        ...
        for ticket in self.dwx.open_orders.keys():
            self.dwx.modify_order(ticket, stop_loss=entry_price - 0.01)  # type: ignore
            ...
            if "modified" in message["message"]:
                ...
                if self.n_modified == self.n:
                    self.modify_duration = (
                        datetime.now(timezone.utc) - self.before_modification
                    ).total_seconds()
    ...
    def on_order_event(self):
        ...
        if len(self.dwx.open_orders) == self.n:
            self.open_duration = (
                datetime.now(timezone.utc) - self.before_open
            ).total_seconds()

        elif len(self.dwx.open_orders) == 0:
            close_duration = (
                datetime.now(timezone.utc) - self.before_close
            ).total_seconds()
            print(
                f"\nopen_duration: {1000*self.open_duration/self.n:.1f} milliseconds per order"
            )
            print(
                f"modify_duration: {1000*self.modify_duration/self.n:.1f} milliseconds per order"
            )
            print(
                f"close_duration: {1000*close_duration/self.n:.1f} milliseconds per order"
            )
